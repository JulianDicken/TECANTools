Utility.extract_datasets_timeseries_multiple <- function(
  data.raw,
  dataset,
  dataset.size
  ) {
  ...
  return(paste0(
    ...
    ":",
    Utility.excel_to_colindex(dataset.items_width), dataset.qidx + (dataset.items_height * dataset.size) - (dataset.qidx - dataset.nidx) + 1
  ))
}
